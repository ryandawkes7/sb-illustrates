        ...
        <div className="col-12 col-md-3">
          {artData.slice(0, 7).map(item => {
            return (
              ...
              >
                <img src={`${process.env.PUBLIC_URL}${item.image}`} className="w-100" style={{pointerEvents: 'none'}}/>
              </button>
            )
          })}
        </div>
...
const ArtModal = (props) => {
  ...
  if (props.show == true) {
    ...
    <div className="modal-bg d-flex justify-content-center align-items-center">
      ...
      {/* Button */}
      <button className="modal-close" onClick={props.hide}>&#x2715;</button>

      {/* Content */}
      <div className="col-11 d-flex justify-content-center">

        {/* Image */}
        <img src={`${process.env.PUBLIC_URL}${props.item.image}`} alt={props.item.title} className="w-auto h-auto" />
